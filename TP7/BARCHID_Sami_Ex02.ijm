...
img_apprentissage = getString("Choisir le path de l'image d'apprentissage à ouvrir", "D:/M2/VISA/VISA/TP7/images_2018/cas_1_dalton42.bmp");

// Nombre de classes défini pour clustering k-means de l'image d'apprentissage (il faut en utiliser 3 pour le clustering avec PCA sur les images de cas 1)
nbClasses = getNumber ("Choisissez le nombre de classes pour le clustering k-means.", 3);

setBatchMode(true);

// Lancer le plugin "k-means" sur l'image d'apprentissage
open(img_apprentissage);
img_apprentissage = getTitle();

// Lancement du PCA sur l'image d'apprentissage
img_apprentissage = usePCA(img_apprentissage);
selectWindow(img_apprentissage);
...
// Application de PCA sur l'image segmentée
img_segmentee = usePCA(img_segmentee);
...
/**
 * Fonction qui utilise l'Analyse en composantes principales sur l'image en paramètre (dans le but d'utiliser k-means par la suite)
 */
 function usePCA(img) {
 	selectWindow(img);

 	// Séparer les trois channels de couleurs (nécessaire pour le plugin "PCA")
 	run("RGB Stack");

 	// Lancer le plugin PCA
 	run("PCA ");

	// Sélectionner la fenêtre qui présente la stack des résultats des PCAs
 	selectWindow("PCA of " + img);

	// Conversion en 8 bits puis reconstruction des stacks en une image RGB)
	run("8-bit");
	run("Stack to RGB");

	// (facultatif) : delete la fenêtre "eigenvalue spectrum" qui ne sert à rien ici
 	close("Eigenvalue spectrum of *");

 	// renvoie du titre de l'image construite à partir de PCA
 	return "PCA of " + img + " (RGB)";
 }
